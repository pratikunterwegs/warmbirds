...
plot(elevation, col = sequential_hcl(20, palette = "Sunset", rev = T))
...
# make grid
hills_sf_buffer = st_transform(hills_sf_buffer, 32643)
hills_grid = st_make_grid(hills_sf_buffer, 
                          cellsize = 100*1e3, square = F, what = "polygons")
# subset by the hills buffer
intersect_grid_hill = st_intersects(hills_sf_buffer, hills_grid)
...
# save to file
st_write(hills_grid, dsn = "data/spatial/hills_sasia_grid_100km.gpkg")
